...
Because the amount of data in the original data set is pretty significant (13737 rows in the training data and 5885 rows in the testing data), especially comparing to the number of variables (only 17 are left after removing non-acceleration and NA variables) no other cross-validation methods (like K-fold or boostrap) are considered anymore.

# Model selection
Several different models are considered for prediction: linear model, tree, random forests, linear discriminant analysis and naive Bayes models. The table below specifies the accuracy of each model on the test set:
...
```
# Conclusion
From the table above I can see that the Random forests model has the best prediction accuracy on the testing data set. This model will be used to predict 20 different test cases from testing.csv file.
